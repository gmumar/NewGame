...
public class UpgradeDialog {
	...
	public static DialogBase CreateDialog(GameLoader gameLoader,
		...
		// Images
		float multiplier = 4;
		if(popQueObject.getItemToBeBoughtName().compareTo(ComponentNames.SPRINGJOINT)==0){
			multiplier=1.5f;
		} else if(popQueObject.getItemToBeBoughtName().compareTo(ComponentNames.AXLE)==0 ||
				popQueObject.getItemToBeBoughtName().compareTo(ComponentNames.TIRE)==0){
			multiplier=3f;
		}
		// Vector2 originalOrigin = null;
		Table currentItem = new Table(skin);
		currentItem.setBackground("partUpgradeBG");
		Image currentItemImage = new Image(
				gameLoader.Assets.getFilteredTexture(ComponentNames.PART_NAME_MAPPING
						.get(popQueObject.getItemToBeBoughtName())
						+ "/level"
						+ Integer.toString(popQueObject.getNextLevel() - 1)
						+ ".png"));
		// Vector2 originalOrigin = new
		...
		float ratio = currentItemImage.getHeight()
				/ currentItemImage.getWidth();
		Label currentItemText = new Label("Level "
				+ Integer.toString(popQueObject.getNextLevel() - 1), skin);
		currentItem.add(currentItemImage).width(Globals.baseSize * multiplier)
				.height(Globals.baseSize * ratio * multiplier).pad(10);
		currentItem.row();
		currentItem.add(currentItemText);

		Table nextItem = new Table(skin);
		nextItem.setBackground("partUpgradeBG");
		...
		nextItem.add(nextItemImage).width(Globals.baseSize * multiplier)
				.height(Globals.baseSize * ratio * multiplier).pad(10);
		nextItem.row();
