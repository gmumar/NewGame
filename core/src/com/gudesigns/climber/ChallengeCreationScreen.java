...
public class ChallengeCreationScreen implements Screen, TwoButtonDialogFlow {
	...
	private void initSendChallengeButton() {
		TextButton createChallenge = new TextButton("set time and send challenge", skin,
				"yesButton");
	...
	private void initSelectionButtons() {
		...
		final ButtonLockWrapper currentTrackButton = AdventureTrackButton
				.create(gameLoader, currentTrack, user.getCurrentTrackMode()==TrackMode.INFINTE, ScreenType.NONE);
		...
		buttonHolder.add(trackHolder).expand().fill();
		
		Table carHolder = new Table(skin);
		
		TextureRegion tr = Assembler.assembleCarImage(gameLoader, user.getCurrentCar(),
				false, false);
		...
		buttonHolder.add(carHolder).expand().fill();
